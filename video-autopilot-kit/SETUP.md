...
2. 照下面每一區的問題填答 —— 或**直接把這個 repo 丟給 Claude / ChatGPT**，說：
   > 「照 `SETUP.md` 一區一區問我問題，用我的答案幫我生成 `profiles/` 裡的檔案」
3. 複製 `config.example.py` → `config.py`，填你自己的路徑

填完 → 你就有一套**你自己的**創作自動化系統，再用 `src/` 的工具跑剪輯 / 發布。

---

## 1️⃣ 品牌 / 頻道 → 生成 `profiles/brand.md`
- 你的頻道名稱 + handle？
- 網站 / 主要連結？
...
→ 然後 `src/` 的 CapCut / ffmpeg 自動化工具就能跑**你的**流程。
